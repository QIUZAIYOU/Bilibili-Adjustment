...
(function () {
    ...
    const arrays = {
        ...
        videoCategoriesActiveClass: ['adjustment_button',
                                     'primary',
                                     'plain']
    }
    const objects = {
    ...
    const styles = {
        ...
        UnlockWebPlayer: 'body.webscreen-fix{padding-top:BODYHEIGHT;position:unset}#bilibili-player.mode-webscreen{height:BODYHEIGHT;position:absolute}#playerWrap{display:none}#danmukuBox{margin-top:0}',
     ...
     */
        async getVueScopeId (selector) {
            ...
            return new Promise((resolve, reject) => {
                ...
                const vueScopeAttrs = attrsArray.filter(attr => attr.name.startsWith('data-v-'))
     ...
     */
        async webfullScreenModeUnlock () {
            ...
            const resetPlayerLayout = async () => {
                if (document.getElementById('UnlockWebPlayerStyle')) document.getElementById('UnlockWebPlayerStyle').remove()
            ...
            utils.insertStyleToDocument('UnlockWebPlayerStyle', styles.UnlockWebPlayer.replace(/BODYHEIGHT/gi, `${bodyHeight}px`))
            ...
            $webEnterButton.addEventListener('click', async () => {
                if (!document.getElementById('UnlockWebPlayerStyle')) utils.insertStyleToDocument('UnlockWebPlayerStyle', styles.UnlockWebPlayer.replace(/BODYHEIGHT/gi, `${bodyHeight}px`))
                $app.prepend($playerWebscreen)
                await modules.locationToPlayer()
            })
     ...
     */
        async insertVideoDescriptionToComment () {
            ...
            const [$videoDescription,
                   $videoDescriptionInfo] = await utils.getElementAndCheckExistence([selectors.videoDescription,
                                                                                     selectors.videoDescriptionInfo])
        ...
        async insertSetSkipTimeNodesButton () {
          ...
          <div id="setSkipTimeNodesButtonTip" class="bpx-player-tooltip-item" style="visibility: hidden; opacity: 0; transform: translate(0px, 0px);">
            ...
            const [$setSkipTimeNodesPopoverHeaderExtra,
                   $setSkipTimeNodesPopoverTips,
                   $setSkipTimeNodesPopoverTipsDetail,
                   $setSkipTimeNodesPopoverRecords,
                   $setSkipTimeNodesInput,
                   $skipTimeNodesRecordsArray,
                   $setSkipTimeNodesPopoverResult,
                   $clearRecordsButton,
                   $saveRecordsButton,
    ...
    */
     ...
     */
        async generatorVideoCategories () {
            ...
            const categoryHasVideoSet = new Set()
            ...
            // })
            for (const [key,
                        value] of Object.entries(objects.videoCategories)) {
                getIndexRecordRecommendVideoHistoryArray.filter(record => {
        ...
        async registerMenuCommand () {
          ...
          <div id="${selectors.dynamicSettingPopover.slice(1)}" class="adjustment_popover" popover>
              <div class="adjustment_popoverTitle">哔哩哔哩动态页设置
                <div class="adjustment-popover-subtitle">（以下设置内容更改即生效，直接关闭本弹窗即可）</div>
              </div>
