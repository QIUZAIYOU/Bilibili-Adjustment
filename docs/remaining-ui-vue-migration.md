...
| E | 播放页设置弹窗（视频/动态） | `settings-dialog` 宿主（`SettingsDialogHost`）+ `SettingsPanel.vue`（Vue 表单） | 大/高 | section/children 显隐、自绘下拉、验证/刷新按钮、tooltip、跨标签同步、主题变量 | ✅ **已 Vue 化且唯一实现**：表单由 `src/ui/settings/SettingsPanel.vue` 渲染，旧命令式渲染器与 `settings_panel`（v3/v2）切换项已删除；详见 docs/settings-panel.md |
