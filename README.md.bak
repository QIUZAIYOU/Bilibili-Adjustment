...
6. **自动跳过广告**：使用 DeepSeek 识别广告（视频必须得有 AI 字幕），需自行填写 DeepSeek 密钥，识别精度完全依赖于 DeepSeek 能力;
...
12. **自动跳过广告**：使用 DeepSeek 识别广告（视频必须得有 AI 字幕），需自行填写 DeepSeek 密钥，识别精度完全依赖于 DeepSeek 能力;
---
...
`06.02 15:52`：新增首页推荐视频付费识别，调用播放器API检测付费视频并在页面标题中添加付费视频标记；抽象 `checkVideoPaid` 函数到 biliApis 服务；付费识别与历史记录解耦，独立为 `markRecommendVideoPaidStatus` 方法随首页模块初始化执行；

`05.29 22:37`：优化火狐浏览器兼容性：
...
`v3.12.0`：优化首页「推荐视频历史记录」：记录携带页面顺序字段，按最新批次排前、同批内保持页面顺序展示；新增写入锁防止快速点击换一换导致的并发写入混乱；修复历史记录弹窗列表排序颠倒问题；  


### 2025

`12.30 16:48`：修复因B站网页元素变化导致的首页「推荐视频历史记录」清空失效的问题;

`12.28 19:14`：调整「广告识别」时机，修复因识别广告阻塞其他功能的问题;

`12.22 23:00`：重做且恢复「自动跳过广告」功能，使用 DeepSeek 识别广告（视频必须得有 AI 字幕），自行填写 DeepSeek 密钥，识别精度完全依赖于 DeepSeek 能力;

`12.11 15:57`：修复「更新弹窗」未正确显示最新版本更新内容的问题;

`12.11 15:44`：优化首页「推荐视频历史记录」中分区模块显示内容，当官方未给出分区名称时显示分区ID;

`12.10 17:22`：优化 URL 的正则表达式;

`11.28 10:41`：优化首页「推荐视频历史记录」显示效果，修复滚动到底部内容显示不全的问题;

`11.27 21:09`：首页「推荐视频历史记录」新增展示UP名（此版本之后添加的记录才有，之前的记录因没存UP名故显示为‘未知作者’），并可使用UP名搜索;

`11.26 20:49`：修复「更新弹窗」未正确显示应有内容的问题，弹窗添加关闭按钮（点击页面中除弹窗外位置依然可以关闭弹窗）；

`11.26 17:58`：修复首页「推荐视频历史记录」数据显示不全的问题，并为「推荐视频历史记录」添加搜索功能，支持关键词搜索；

`10.28 22:04`：修复字幕相关功能（适配B站新版字幕功能，现在将强制开启「中文AI」字幕，无视视频作者默认开启的字幕）；

`10.27 10:00`：修复字幕相关功能（适配B站新版字幕功能）；

`08.05 08:30`：适配「list」开头的视频播放链接(个人主页➡️播放全部)；
...
- [设置项修改指导文档](docs/settings-modification-guide.md)：新增或修改设置项时的完整流程规范，避免选择器未注册、配置未保存等常见问题。
