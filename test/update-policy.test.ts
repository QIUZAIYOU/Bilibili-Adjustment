...
test('formatPendingUpdateHint：有新版本用「- 有新版本 vX -」，覆盖发布用「- 内容补丁可更新 -」，都没有则空串', () => {
    ...
    assert.equal(formatPendingUpdateHint(null, true), '- 内容补丁可更新 -')
    assert.equal(formatPendingUpdateHint('', true), '- 内容补丁可更新 -')
