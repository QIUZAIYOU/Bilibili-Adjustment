    ...
    async identifyAdvertisementTimestamps (subtitlesJsonString) {
        ...
        try {
            ...
            if (error.response) {
                ...
                switch (status) {
                    case 400:
                        errorMessage = 'OpenAI API请求格式错误：请求体格式错误。解决方法：请根据错误信息提示修改请求体'
                        break
                    case 401:
                        errorMessage = 'OpenAI API认证失败：API key错误，认证失败。解决方法：请检查您的API key是否正确，如没有API key，请先创建API key'
                        break
                    case 429:
                        errorMessage = 'OpenAI API请求速率达到上限：请求速率（TPM或RPM）达到上限。解决方法：请合理规划您的请求速率'
                        break
                    case 500:
                        errorMessage = 'OpenAI API服务器故障：服务器内部故障。解决方法：请等待后重试。若问题一直存在，请联系我们解决'
                        break
                    case 503:
                        errorMessage = 'OpenAI API服务器繁忙：服务器负载过高。解决方法：请稍后重试您的请求'
                        break
                    default:
                        errorMessage = `OpenAI API请求失败，状态码：${status}`
                }
                this.#logger.error(errorMessage, error)
            } else {
                this.#logger.error('字幕分析失败', error)
