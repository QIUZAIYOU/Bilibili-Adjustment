...
/** 远程提示词文件（与 meta.js 同目录；扩展名 .js 以命中服务器的 CORS 规则） */
export const REMOTE_PROMPT_URL = 'https://www.asifadeaway.com/UserScripts/bilibili/ad-detection-prompt.js'
...
const fetchRemote = async (): Promise<RemotePromptPayload | null> => {
    ...
    try {
        ...
        logger.debug('远程提示词不可用，改用兜底：' + (error instanceof Error ? error.message : String(error)))
        return null
    } finally {
        clearTimeout(timer)
    }
