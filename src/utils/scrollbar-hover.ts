...
const PAGE_SCROLLBAR: unique symbol = Symbol('page-scrollbar')
const easeOutCubic = (t: number): number => 1 - Math.pow(1 - t, 3)
/**
 * 本模块的滚动条加宽完全依赖 Chromium 的 ::-webkit-scrollbar（Firefox 不支持，
 * 相关样式会静默失效）。Firefox 下直接跳过，避免白跑一轮观察器与逐帧重排。
 */
const isWebkitScrollbarSupported = (): boolean => !/firefox/i.test(navigator.userAgent)
/** 悬停目标：具体元素，或页面（文档）滚动条这一特殊标记 */
type ScrollbarTarget = Element | typeof PAGE_SCROLLBAR
/** 加宽/恢复动画状态（同一时间只有一个目标持有悬停标记） */
interface ScrollbarHoverState {
    el: ScrollbarTarget | null
    width: number
    to: number
    raf: number
    start: number
...
export const initScrollbarHoverWidening = (): void => {
    ...
    const active: ScrollbarHoverState = { el: null, width: SCROLLBAR_BASE_WIDTH, to: SCROLLBAR_BASE_WIDTH, raf: 0, start: 0, from: 0 }
    // !important 保证覆盖 B站 自身的滚动条宽度样式；属性选择器特异性高于基础规则
    const writeWidthRule = (width: number): void => {
    ...
    const applyHoverMark = (target: ScrollbarTarget): void => {
    ...
    const removeHoverMark = (target: ScrollbarTarget): void => {
        if (target === PAGE_SCROLLBAR) {
    ...
    const startWiden = (target: ScrollbarTarget): void => {
    ...
    const rendersScrollbar = (overflow: string, scrollSize: number, clientSize: number): boolean =>
        scrollSize > clientSize + 1 && overflow !== 'visible' && overflow !== 'hidden' && overflow !== 'clip'
    // 按 thumb 几何位置判断光标是否落在滑块上（轨道命中不触发加宽）
    const isCursorOverThumb = (el: Element, axis: 'x' | 'y', clientX: number, clientY: number, trackStartOverride?: number): boolean => {
    ...
    const findHoveredScrollbar = (clientX: number, clientY: number): ScrollbarTarget | null => {
        // 页面内元素滚动条优先：从光标所在元素向上遍历
        const cursorElement = document.elementFromPoint(clientX, clientY)
        if (cursorElement) {
            let el: Element | null = cursorElement
