...
/** 归一化后的请求错误（刻意对齐 axios 形状：response.status / code） */
export interface HttpError extends Error {
    response?: { status: number; data: unknown }
    code?: string
}
const buildError = (message: string, extra: Partial<HttpError> = {}): HttpError => {
...
/** 统一响应形状（与 axios 对齐的 data/status/headers） */
...
export const httpRequest = async (url: string, options: HttpRequestOptions = {}): Promise<HttpResponse> => {
    ...
    for (;;) {
        ...
        try {
            ...
            if (httpError.response) throw error
            if (signal?.aborted) {
...
export const httpGet = (url: string, options: HttpRequestOptions = {}): Promise<HttpResponse> => httpRequest(url, { ...options, method: 'GET' })
export const httpPost = (url: string, body: string | object | null, options: HttpRequestOptions = {}): Promise<HttpResponse> => httpRequest(url, { ...options, method: 'POST', body })
