...
let savedScrollState: { overflow: string; paddingRight: string } | null = null
...
export interface DialogA11yOptions {
    /** 标题元素 id（aria-labelledby） */
    labelledBy?: string | null
    /** Esc 回调（默认无操作，调用方通常关闭弹窗） */
    onEscape?: (() => void) | null
}
export const applyDialogA11y = (root: HTMLElement, options: DialogA11yOptions = {}): (() => void) => {
    const { labelledBy = null, onEscape = null } = options
    const activeElement = document.activeElement
    const previouslyFocused = activeElement && activeElement !== document.body ? (activeElement as HTMLElement) : null
    root.setAttribute('role', 'dialog')
    root.setAttribute('aria-modal', 'true')
    if (labelledBy) root.setAttribute('aria-labelledby', labelledBy)
    if (!root.hasAttribute('tabindex')) root.setAttribute('tabindex', '-1')
    const getFocusable = (): HTMLElement[] => (Array.from(root.querySelectorAll(FOCUSABLE_SELECTOR)) as HTMLElement[])
