...
/** 注册作用域级自动清理 */
const disposeOnScope = (cleanup: () => void): void => {
    onScopeDispose(() => {
        try {
            cleanup()
        } catch {
...
export interface UseConfigResult {
    /** 当前值（首次异步读取完成前为 undefined） */
    value: ShallowRef<unknown>
    /** 首次读取是否完成 */
    loading: ShallowRef<boolean>
    /** 写入配置（本地立即同步 value；跨标签页由 config:changed 事件同步） */
    set: (next: unknown) => Promise<void>
}
export const useConfig = (key: string): UseConfigResult => {
    ...
    load()
    const off = eventBus.on(EVENT_NAMES.CONFIG_CHANGED, (_ctx, ...args: unknown[]) => {
        const payload = args[0] as { key?: string; value?: unknown } | undefined
...
export const useEvent = (eventName: string, handler: (context: unknown, ...payload: unknown[]) => unknown): void => {
    const off = eventBus.on(eventName, handler)
    disposeOnScope(off)
}
/** 当前实际生效的主题 id（night/light/dark），随主题切换自动更新 */
export const useCurrentTheme = (): ShallowRef<string> => {
    const theme = shallowRef<string>(ThemeManager.getTheme())
