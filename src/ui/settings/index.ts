...
export interface MountVueSettingsOptions {
    schema?: unknown[]
...
export interface VueSettingsPanelHandle {
    bridge: {
        schema: unknown
        configs: Record<string, unknown>
        dynamicOptions: Record<string, unknown>
...
export const mountVueSettingsPanel = async (mountEl: HTMLElement | null, options: MountVueSettingsOptions = {}): Promise<VueSettingsPanelHandle> => {
    ...
    const bridge = {
        schema: markRaw(schema as object),
    ...
    const app = createApp(PanelComponent, {
        ...
        onValidate: (key: string) => onValidate?.(key),
        onRefresh: (key: string) => onRefresh?.(key)
    })
