...
let dialogSeq = 0
/** 弹窗操作按钮 */
export interface AdjustmentDialogAction {
    text: string
    /** 按钮类型（对应 .adjustment-button 的修饰类） */
    type?: string
    className?: string
    onClick?: (dialog: AdjustmentDialogInstance) => void
}
/** 弹窗内容：HTML 字符串、Node，或渲染函数（可返回清理函数，如 Vue 的 unmount） */
export type AdjustmentDialogContent = string | Node | ((body: HTMLElement) => void | (() => void))
/** openAdjustmentDialog 的选项 */
export interface AdjustmentDialogOptions {
    key?: string
    title?: string
    subtitle?: string
    /** 标题右侧的小标签（如「测试」） */
    titleTag?: string
    content?: AdjustmentDialogContent
    actions?: AdjustmentDialogAction[]
    width?: number | string
    className?: string
    closable?: boolean
    headerExtra?: string | Node | null
    /** > 0 时关闭后保留实例（缓存期内复用，不重建内容） */
    keepAliveMs?: number
    /** > 0 时自动关闭（ms） */
    autoClose?: number
    onClosed?: () => void
...
/** 弹窗元素（带轻量关闭清理句柄） */
type DialogRoot = HTMLElement & { __popoverDismissCleanup?: (() => void) | null }
/** 弹窗实例 */
export interface AdjustmentDialogInstance {
    key: string
    root: DialogRoot
    header: HTMLElement
    body: HTMLElement
...
interface DialogEntry {
    instance: AdjustmentDialogInstance | null
...
const dialogInstances = new Map<string, DialogEntry>()
export const openAdjustmentDialog = (options: AdjustmentDialogOptions = {}): AdjustmentDialogInstance => {
    const {
        key = '',
        title = '',
        subtitle = '',
    ...
    if (key && dialogInstances.has(key)) {
        const prevInstance = dialogInstances.get(key)?.instance
        if (prevInstance && !prevInstance._destroyed) {
            prevInstance.open()
            return prevInstance
        }
    ...
    ensureDialogStyle()
    const root = document.createElement('div') as DialogRoot
    ...
    // ---------- 实例 ----------
    const entry: DialogEntry = { instance: null, timer: null }
    let autoCloseTimer: ReturnType<typeof setTimeout> | null = null
    let destroyed = false
    const cancelAutoClose = (): void => {
        if (autoCloseTimer) {
            clearTimeout(autoCloseTimer ?? undefined)
            autoCloseTimer = null
        }
    }
    const scheduleLazyDestroy = (): void => {
    ...
    let a11yCleanup: (() => void) | null = null
    const setupDialogA11y = (): void => {
    ...
    const teardownDialogA11y = (): void => {
        if (!a11yCleanup) return
        a11yCleanup()
        a11yCleanup = null
    }
    const instance: AdjustmentDialogInstance = {
        key,
        root,
        header,
        body,
    ...
    root.addEventListener('toggle', (e: Event) => {
        if ((e as ToggleEvent).newState !== 'closed' || destroyed) return
