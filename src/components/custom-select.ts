...
type SelectHost = HTMLElement & {
    /** 关闭菜单时的监听清理句柄 */
    __adjSelectCleanup?: (() => void) | null
    /** 键盘高亮的下标（-1 表示无） */
    __adjSelectIndex?: number
    /** 同步 aria-expanded 的 class 观察器 */
    __adjSelectObserver?: MutationObserver | null
}
/** 确保样式注入一次（幂等） */
 ...
 */
const buildMenuItems = (select: HTMLSelectElement, menu: HTMLElement, onPick: (value: string, text: string | null) => void): void => {
    menu.textContent = ''
 ...
 * 关闭已打开的菜单并归还焦点
 */
const closeMenu = (host: SelectHost, restoreFocus = false): void => {
    const menu = host.querySelector('.adj-select-menu') as HTMLElement | null
    const trigger = host.querySelector('.adj-select-trigger') as HTMLElement | null
 ...
 */
const commit = (host: SelectHost, value: string): void => {
 ...
 */
const getScrollContainer = (el: Element): HTMLElement => {
...
const openMenu = (host: SelectHost): void => {
    const select = host.querySelector('select') as HTMLSelectElement | null
    const trigger = host.querySelector('.adj-select-trigger') as HTMLElement | null
    const menu = host.querySelector('.adj-select-menu') as HTMLElement | null
    ...
    const selectedIndex = allItems.findIndex(el => (el as HTMLElement).dataset.value === select.value)
    ...
    const onDocPointerDown = (e: Event): void => {
        if (!host.contains(e.target as Node)) closeMenu(host)
    }
    const onPageScroll = (e: Event): void => {
        if (e.target === document || e.target === document.documentElement) closeMenu(host)
    }
    const onResize = (): void => closeMenu(host)
    // 事件名断言为 string：走 addEventListener 的通用重载（专用重载的 ev 形参更窄，宽形参无法逆变匹配）
    document.addEventListener('pointerdown' as string, onDocPointerDown, true)
    window.addEventListener('scroll' as string, onPageScroll, true)
    window.addEventListener('resize' as string, onResize)
    document.addEventListener('keydown' as string, onKeydown, true)
    host.__adjSelectCleanup = () => {
        document.removeEventListener('pointerdown' as string, onDocPointerDown, true)
        window.removeEventListener('scroll' as string, onPageScroll, true)
        window.removeEventListener('resize' as string, onResize)
        document.removeEventListener('keydown' as string, onKeydown, true)
    }
}
/** 菜单内键盘导航 */
const onKeydown = (e: Event): void => {
    const key = (e as KeyboardEvent).key
    const host = (e.target as Element | null)?.closest?.('.adjustment-select.adj-select-enhanced.open') as SelectHost | null
    if (!host) return
    ...
    switch (key) {
        ...
        case ' ': {
            e.preventDefault()
            e.stopPropagation()
            const activeItem = index >= 0 ? items[index] as HTMLElement : null
            if (activeItem && activeItem.dataset.value !== undefined) {
                commit(host, String(activeItem.dataset.value))
            } else {
    ...
    if (index >= 0 && items[index]) {
        ...
        select?.setAttribute('data-adj-active-value', (items[index] as HTMLElement).dataset.value ?? '')
    }
}
/**
 * 刷新单个增强容器的视觉（label / disabled），由外部修改 select 后调用
 */
const refreshHost = (host: SelectHost): void => {
    const select = host.querySelector('select') as HTMLSelectElement | null
    const trigger = host.querySelector('.adj-select-trigger') as HTMLButtonElement | null
 ...
 */
export const enhanceCustomSelects = (root: Element | null | undefined): void => {
    if (!root) return
    ensureStyle()
    root.querySelectorAll('.adjustment-select').forEach(node => {
        const container = node as SelectHost
        const select = container.querySelector('select[data-config-type="select"]') as HTMLSelectElement | null
 ...
 * @param {HTMLElement} root - 弹窗或任意容器
 */
export const refreshCustomSelects = (root: Element | null | undefined): void => {
    if (!root) return
