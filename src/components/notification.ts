...
`
let _notificationContainer: HTMLDivElement | null = null
let _styleInjected = false
const _ensureNotificationContainer = (): HTMLDivElement => {
...
const _levelIcons: Record<string, string> = {
    info: '',
...
const _dismiss = (el: Element | null): void => {
...
export const notification = {
    /** 显示一条通知（返回通知元素，调用方可自行移除） */
    show ({ level = 'info', title = '', message = '', duration = NOTIFICATION_DURATION }: NotificationOptions = {}): HTMLDivElement {
        ...
        const bar = el.querySelector('.ba-notification__progress') as HTMLElement
    ...
    warn (title: string, message: string): HTMLDivElement {
        return this.show({ level: 'warn', title, message })
    },
    error (title: string, message: string): HTMLDivElement {
