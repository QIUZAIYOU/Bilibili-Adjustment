/**
 * 评论区简介条目（shadow 富文本）渲染函数。
 * 从占位符模板（getTemplates.replace）改为直接插值渲染，结构与旧模板逐字一致：
 * - styleCss：注入 shadow 的样式（stylesV2.videoCommentDescription）
 * - avatarDataUri：UP 头像 data URI
 * - bodyHtml：已处理（转义 + 链接化）的简介富文本
 */
export const renderVideoDescription = ({ styleCss = '', avatarDataUri = '', bodyHtml = '' }) =>
    '<bili-adjustment-comment-thread-renderer id="adjustment-comment-description" bilibili-adjustment-element><style>' + styleCss + '</style><bili-adjustment-comment-renderer><div id="bili-adjustment-body"><a id="bili-adjustment-user-avatar"><bili-adjustment-avatar><img id="bili-adjustment-avatar-picture"src="' + avatarDataUri + '"alt="' + avatarDataUri + '"></bili-adjustment-avatar></a><div id="bili-adjustment-main"style="width:100%"><div id="bili-adjustment-header"><bili-adjustment-comment-user-info><div id="bili-adjustment-info"><div id="bili-adjustment-user-name"><a href="#"onclick="event.preventDefault()">视频简介君(ﾉ≧∀≦)ﾉ</a></div><div id="bili-adjustment-user-badge">Bilibili调整</div></div></bili-adjustment-comment-user-info></div><div id="bili-adjustment-content"><bili-adjustment-rich-text><p id="bili-adjustment-contents">' + bodyHtml + '</p></bili-adjustment-rich-text></div></div></div></bili-adjustment-comment-renderer><div id="bili-adjustment-spread-line"></div></bili-adjustment-comment-thread-renderer>'
