export const ifTrue = (condition: unknown, styles: string): string => condition ? styles : ''
export const cx = (...styles: Array<string | false | null | undefined>): string => styles.filter(Boolean).join(' ')
export const breakpoint = {
    sm: '@media (max-width: 576px)',
    md: '@media (max-width: 768px)',
    lg: '@media (max-width: 992px)'
}
export const scrollbarStyle = (color = 'var(--adj-brand)'): string => `
    ::-webkit-scrollbar {
        width: 6px;
        height: 6px;
    }
...
`
export const popoverBaseStyle = (width = '520px'): string => `
    position: fixed;
    inset: 0;
    margin: auto;
...
`
export const buttonStyle = (variant = 'primary'): string => {
