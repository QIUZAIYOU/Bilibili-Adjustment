...
const isStylusNightActive = (): boolean =>
    [...document.querySelectorAll(STYLUS_STYLE_SELECTOR)].some(el => {
        ...
        return !styleEl.disabled && !styleEl.sheet?.disabled && (styleEl.textContent || '').includes(STYLUS_NIGHT_MARK)
    })
const applyStateNow = async (): Promise<void> => {
...
export const initStylusNightFollowing = (): void => {
    if (started) return
    started = true
    applyState()
