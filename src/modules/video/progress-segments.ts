...
import type { SkipSegment } from './skip-manager/pure'
...
const removeTints = (): void => {
    document.querySelectorAll('.' + TINT_CLASS + '-layer').forEach(el => el.remove())
}
/** 把片段区间绘制到官方进度条上 */
const renderTints = async (segments: SkipSegment[]): Promise<void> => {
    ...
    if (!duration) return
    removeTints()
    const valid = (segments || []).filter(seg => Number.isFinite(seg.start) && Number.isFinite(seg.end) && seg.end > seg.start)
...
const isTintEnabled = async (): Promise<boolean> => {
    try {
...
const refresh = async (): Promise<void> => {
    ...
    try {
        ...
        // 兼容两种缓存形态：{ segments } 或片段数组本身
        const entry = cached as { segments?: unknown } | null | undefined
        const rawSegments = entry && (entry.segments || entry)
        const segments = sanitizeSegments(rawSegments || [])
        await renderTints(segments)
    } catch (error) {
        logger.debug('读取片段缓存失败，跳过进度条染色:', error instanceof Error ? error.message : String(error))
    }
}
/** 启动：官方进度条插入后绘制，并在时长/视频切换时重绘 */
export const initProgressSegmentTint = (): void => {
    if (started) return
    started = true
    ensureStyle()
    ...
    const attach = () => {
        ...
        if (video) {
            // options 提取为变量：'durationchange' 专用重载的 options 是 EventListenerOptions（不含 passive）
            const passiveOptions: AddEventListenerOptions = { passive: true }
            video.addEventListener('durationchange', refresh, passiveOptions)
            video.addEventListener('loadedmetadata', refresh, passiveOptions)
        }
        refresh()
        return true
    }
