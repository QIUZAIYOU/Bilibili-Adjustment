 ...
 */
/** 单个跳过片段（summary 为识别结果或手动备注） */
export interface SkipSegment {
    start: number
    end: number
    summary?: string
}
/** 缓存的跳过片段条目（与 createCacheEntry 结构一致） */
export interface SkipCacheEntry {
    bvid?: string
    segments?: SkipSegment[]
    uploader_uid?: number | null
    verified_by?: Array<number | null>
    version?: number
    last_updated?: number
    /** 锁定标记：服务端与本地面板分别用 1/0 与布尔表达 */
...
/** 创建标准缓存数据结构 */
export const createCacheEntry = (bvid: string, segments?: SkipSegment[] | null, uid: number | null = null): SkipCacheEntry => ({
    bvid,
    segments: segments || [],
    uploader_uid: uid,
    verified_by: uid ? [uid] : [],
...
export const getCurrentUid = (): number | null => {
    try {
 ...
 */
export const validateSegment = (newSeg: SkipSegment, existingSegments: SkipSegment[]): string | null => {
 ...
 *      否则调用方传入的数组元素会被就地修改。
 */
export const mergeSegments = (segments: SkipSegment[]): SkipSegment[] => {
 ...
 */
export const canUpdateCache = (cached: SkipCacheEntry | null | undefined, _currentUid?: number | null): boolean => {
...
export const parseDuration = (str: string): number | null => {
...
/** createSkipMatcher 的选项 */
export interface SkipMatcherOptions {
    epsilon?: number
    skipBackTolerance?: number
}
/** 跳过片段匹配器（纯函数 + 闭包状态） */
export interface SkipMatcher {
    sortedSegments: SkipSegment[]
    lastEnd: number
    match: (currentTime: number) => SkipMatchResult | null
    isFinishedAt: (currentTime: number) => boolean
    readonly skippedKey: string | null
}
export const createSkipMatcher = (segments: SkipSegment[], options: SkipMatcherOptions = {}): SkipMatcher => {
 ...
 */
export const sanitizeSegments = (segments: unknown): SkipSegment[] => {
    const list = Array.isArray(segments) ? segments : []
    const valid: SkipSegment[] = []
    for (const raw of list) {
        const item = raw as { start?: unknown; end?: unknown } | null | undefined
        if (!item || !Number.isFinite(Number(item.start)) || !Number.isFinite(Number(item.end))) continue
        const start = Number(item.start)
        const end = Number(item.end)
        if (!(end > start)) continue
        // 与原实现一致：保留条目上的其它字段（如 summary），仅归一化 start/end
        valid.push({ ...(item as Record<string, unknown>), start, end } as SkipSegment)
    }
