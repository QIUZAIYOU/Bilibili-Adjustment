...
export interface LoadCacheResult {
    cached: SkipCacheEntry | null
    segments: SkipSegment[]
    fromRemote: boolean
}
/** commitBatch 的单集项（replaceSegments 为 null 表示追加模式） */
export interface CommitBatchItem {
    episodeId: string
    cached: SkipCacheEntry | null | undefined
    addSegments?: SkipSegment[]
    replaceSegments?: SkipSegment[] | null
}
/** commitCache 的结果 */
export interface CommitCacheResult {
    cached: SkipCacheEntry
    segments: SkipSegment[]
    canUpdate: boolean
}
/** 读取某视频缓存：本地优先，未命中回退远程（命中写本地）。返回统一装载结果 */
export const loadCache = async (env: SkipManagerEnv, bvid: string): Promise<LoadCacheResult> => {
    ...
    let remote: SkipCacheEntry | null = null
    try {
        ...
        if (resp.ok) {
            ...
            if (result.ok && result.data && Array.isArray(result.data.segments) && result.data.segments.length > 0) {
                remote = result.data as SkipCacheEntry
            }
        }
    } catch (error) {
...
export const loadEpisodesCache = async (env: SkipManagerEnv, episodeIds: string[]): Promise<Map<string, SkipCacheEntry | null>> => {
    const result = new Map<string, SkipCacheEntry | null>()
    ...
    for (let i = 0; i < missing.length; i += 4) {
        await Promise.all(missing.slice(i, i + 4).map(async id => {
            try {
                ...
                const data = payload && payload.ok ? (payload.data as SkipCacheEntry) : null
                if (data && Array.isArray(data.segments) && data.segments.length > 0) {
                    await env.storage.adCacheSet(id, data).catch(() => {})
                    result.set(id, data as SkipCacheEntry)
 ...
 */
export const commitBatch = async (env: SkipManagerEnv, items: CommitBatchItem[]): Promise<number> => {
    let success = 0
...
export const detectSubtitles = async (env: SkipManagerEnv, bvid: string): Promise<boolean> => {
    const { biliApis } = env
...
/** 提交缓存：本地 + 远程，返回新缓存条目 */
export const commitCache = async (env: SkipManagerEnv, bvid: string, previousCached: SkipCacheEntry | null | undefined, finalSegments: SkipSegment[]): Promise<CommitCacheResult> => {
    const uid = env.uidProvider()
    const merged = [...finalSegments].sort((a, b) => a.start - b.start)
    const entry: SkipCacheEntry = {
        bvid,
        segments: merged,
        uploader_uid: uid,
        verified_by: uid ? [uid] : [],
