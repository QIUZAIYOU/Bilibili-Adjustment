...
export const uiButtonsFeatures = {
    ...
    async locateButtonClick (this: UiButtonsContext): Promise<void> {
        const miniOpenBtn = document.querySelector('.mini-player-window[title="点击打开迷你播放器"]') as HTMLElement | null
        ...
        return wait(50)
            ...
            .then(() => {
                const closeBtn = document.querySelector('#mirror-vdcon .mini-player-window.fixed-sidenav-storage-item') as HTMLElement | null
                closeBtn?.click()
            })
    },
